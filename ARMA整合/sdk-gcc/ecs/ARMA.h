...
//        std::cout << std::endl;
        ...
        if(this->q == 0){
            ...
            int errSize = this->array.size() - this->p;
            std::vector<double> errAll(errSize + count);


            for(int i = 0; i < errAll.size(); i++) {
                double randValue = gaussrand1();
                errAll[i] = std::sqrt(var) * gaussrand1();
            }

            for(int i = 0; i< count; i++){
                int size = arrayTemp.size();
                predict[i] = errAll[errSize + i];
            ...
            //所有err
            int errSize = this->array.size();
            std::vector<double> errDataALL(errSize + count);
            for(int i = 0; i < errDataALL.size(); i++){
                errDataALL[i] = std::sqrt(var) * gaussrand1();
            }

            //有用的err
            for(int i = 0; i < errLength; i++){
                errData[i] = errDataALL[errSize - this->q + i];
            }
            ...
            std::vector<double> errDataALL(errSize + count);
            for(int i = 0; i < errDataALL.size(); i++){
                errDataALL[i] = gaussrand1()*std::sqrt(var);
            }
            ...
            for(int i = 0; i < errLength; i++){
                errData[i] = errDataALL[errDataALL.size() - errLength + i];
            }
            for(int i = 0; i < count; i++){
                int size = arrayTemp.size();
